...
public class SMG : MonoBehaviour {
	...
	void Start () {
		PistolMainAudio = GameObject.Find("SShooT").GetComponent<AudioSource>();
	...
	void Update () {
		...
		if (Input.GetKey(KeyCode.Mouse0) && cooldownRemaining <= 0 && currentbulletsInHolder > 0) 
		{
			...
			//Animation
			//GameObject.Find("Pistol").GetComponent<Animation>().Play("Shoot");
			//Sound
		...
		if (Input.GetKey(KeyCode.Mouse0) && cooldownRemaining <= 0 && currentbulletsInHolder == 0) 
		{
			//GameObject.Find("Pistol").GetComponent<Animation>().Play("NoAmmo");
		...
		if(currentbulletsInHolder <= bulletsInHolder-1 && Holders > 0 && Input.GetKeyDown(KeyCode.R) && cooldownRemaining <= 0)
		{
			...
			//GameObject.Find("Pistol").GetComponent<Animation>().Play("Reload");
